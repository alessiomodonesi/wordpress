...
<div class="container-fluid">
  <section>
    ...
    <div class="demo-html">
      <table id="fascicoli_specialista" class="table table-striped table-bordered" style="width:100%">
        <thead>
        <tr>
          <th>Id</th>
          <th>Nickname</th>
        ...
        <?php
          ...
          INNER JOIN UFFICIO ON UFFICIO.ID=FASCICOLO.id_ufficio
           INNER JOIN STATO ON STATO.id=FASCICOLO.stato
           INNER JOIN REFERENTE ON REFERENTE.id=FASCICOLO.referente
           INNER JOIN CASO ON CASO.id=FASCICOLO.caso
           INNER JOIN(SELECT fs2.id_fascicolo as id_fascicolo, fs2.id_specialista as id_specialista, fs2.data_assegnazione as data_assegnazione,fs2.data_conclusione as data_conclusione, fs2.note as note from FASCICOLO_SPECIALISTA fs2 inner join
(select id_fascicolo, MAX(numero_specialista) as ordine_specialista from FASCICOLO_SPECIALISTA group by(id_fascicolo)) fs
on fs2.id_fascicolo=fs.id_fascicolo AND fs2.numero_specialista =ordine_specialista WHERE fs2.id_specialista=".$id_specialista.") gg on gg.id_fascicolo=FASCICOLO.id");

        ?>
        <?php $__currentLoopData = $result; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <tr>

            <td><a href="/single_fascicolo?id=<?php echo $row->id; ?>"><?php echo $row->id; ?></a> </td>
            <td>
              <?php echo $row->nickname; ?>

            </td>
            <td><?php echo $row->data_apertura; ?></td>
            <td><?php echo $row->data_chiusura; ?></td>
            <td><?php echo $row->stato; ?></td>
            <td><?php echo $row->ufficio; ?></td>
            <td><?php echo $row->persone_gestite; ?></td>
            <td><?php echo $row->nome_ref; ?> <?php echo $row->middle_name_ref; ?> <?php echo $row->cognome_ref; ?></td>
            <td><?php echo $row->caso; ?></td>
            <td><?php echo $row->link; ?></td>
            <td><?php echo $row->log_utente; ?></td>
            <td><?php echo $row->log_azione; ?></td>
            <td><?php echo $row->date_time_record_insert; ?></td>
            <td><?php echo $row->date_time_record_update; ?></td>
