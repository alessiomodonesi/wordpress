...
<script type="text/javascript" language="javascript" class="init">
  ...
  var editor_2; // use a global for the submit and return data rendering in the examples
  jQuery(document).ready(function($){
    editor_2 = new $.fn.dataTable.Editor( {
      ajax:{url:"../../../Editor-PHP/controllers/storico_specialisti.php",
        data:{id_fascicolo:<?php echo $id_fascicolo; ?>}},
      table: "#storico_specialisti",
      fields: [
        {
          label:"specialista:",
          name:"id_specialista",
          type:"select",
          options:get_specialisti()

        },
        {
          label: "numero specialista:",
          name:"numero_specialista"
        }

      ]
    } );
    function get_specialisti(){
      $.ajax ( {
        url         : '../../../Editor-PHP/controllers/specialista.php',
        dataType    : 'json',
        type        : 'post',
        success     : function ( jsonData ) {
          var options = jsonData.data.map( item => {
            return { value: item.id,label: item.nome+' '+item.middle_name+' '+item.cognome+'-'+item.codice_fiscale };
          } );
          editor_2.field('id_specialista').update(options);
        }
      });
    } ;


    var table_2 = $('#storico_specialisti').DataTable( {
      language: {
      ...
      columns: [
        ...
        {
          data: "specialista_nome",
          render: function ( data, type, row ) {
            return '<a href="/single_specialista/?id='+row.id_specialista+'">'+row.specialista_nome+' '+row.specialista_middle_name+' '+row.specialista_cognome+'</a>';
        ...
        {
          data: "data_conclusione"
        },
        {
          data:"numero_specialista"
        },
        {
          data:"accettato"
        },
    ...
    // Display the buttons
    new $.fn.dataTable.Buttons( table_2, [
      { extend: "create", editor: editor_2 }
    ] );

    table_2.buttons().container()
      .appendTo( $('.col-md-6:eq(0)', table_2.table().container() ) );
  } );
...
<div class="container-fluid">
  <section>
    ...
    <div class="demo-html">
      <table id="storico_specialisti" class="table table-striped table-bordered" style="width:100%">
        <thead>
        <tr>
          <th>Specialista</th>
          <th>Data assegnazione</th>
          <th>Data conclusione</th>
          <th>Numero specialista</th>
          <th>Accettato</th>
          <th>Log utente</th>
          <th>Log azione</th>
